...
def set_heater():
    """
    API endpoint to set the heater configuration based on the selected action.
    """
    data = request.json
    ...
    # Determine the local file path based on the action
    if action == "low":
        local_file_path = "bosminer-low.toml"
    elif action == "medium":
        local_file_path = "bosminer-medium.toml"
    elif action == "high":
        local_file_path = "bosminer-high.toml"
    else:
    ...
    try:
        ...
        return jsonify({"message": f"Heater '{heater_name}' configuration updated to '{action}' mode"}), 200
    except Exception as e:
